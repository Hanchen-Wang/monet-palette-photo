...
2. Route to one mode and one presentation profile. Read only the selected mode reference and, for `sparse_social`, [references/sparse-social.md](references/sparse-social.md). Confirm mode, strength, and profile in one sentence.
...
6. Use the host's available reference-image generation or editing tool. Pass the source image as a reference; use a mask or deterministic compositing when exact pixel preservation is required and supported.
...
2. `Mode: <mode> · Strength: <M1–M4> · Profile: <immersive|sparse_social>`.
